...
struct sFMindex gnFMidx,grFMidx;
...
//		printf("%s\n",memptr->b);
	...
	if(file)
	{
		...
		len = ftell(file);//获取文件长度，（得到文件位置指针当前位置相对于文件首的偏移字节数）
		rewind(file);//将文件内部的指针重新指向一个流的开头
		FMidx.c = (uint32_t *)malloc(len);//申请内存空间，len*sizeof(char)是为了更严谨，16位上char占一个字符，其他机器上可能变化
		//用malloc申请的内存是没有初始值的，如果不赋值会导致写入的时候找不到结束标志符而出现内存比实际申请值大，写入数据后面跟随乱码的情况
		memset(FMidx.c,0,len);//将内存空间都赋值为‘\0’

		fread(FMidx.c,1,len,file);
		FMidx.sa_gap = FMidx.c[4];
		FMidx.occ_gap = FMidx.c[5];
//		for(uint32_t i = 0; i < len / 4; i++)
//		{
//			printf("%d ",(memptr->c)[i]);
//		}
